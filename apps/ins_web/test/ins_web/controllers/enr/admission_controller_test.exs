defmodule InsWeb.ENR.AdmissionControllerTest do
  use InsWeb.ConnCase
  ...
  def fixture(:admission) do
    {:ok, admission} = ENR.create_admission(@create_attrs)
    admission
  end

  describe "index" do
    test "lists all admissions", %{conn: conn} do
      conn = get conn, enr_admission_path(conn, :index)
  ...
  describe "new admission" do
    test "renders form", %{conn: conn} do
      conn = get conn, enr_admission_path(conn, :new)
  ...
  describe "create admission" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, enr_admission_path(conn, :create), admission: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == enr_admission_path(conn, :show, id)

      conn = get conn, enr_admission_path(conn, :show, id)
      assert html_response(conn, 200) =~ "Show Admission"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, enr_admission_path(conn, :create), admission: @invalid_attrs
      assert html_response(conn, 200) =~ "New Admission"
    end
  end
  ...
  describe "edit admission" do
    ...
    test "renders form for editing chosen admission", %{conn: conn, admission: admission} do
      conn = get conn, enr_admission_path(conn, :edit, admission)
  ...
  describe "update admission" do
    ...
    test "redirects when data is valid", %{conn: conn, admission: admission} do
      conn = put conn, enr_admission_path(conn, :update, admission), admission: @update_attrs
      assert redirected_to(conn) == enr_admission_path(conn, :show, admission)

      conn = get conn, enr_admission_path(conn, :show, admission)
    ...
    test "renders errors when data is invalid", %{conn: conn, admission: admission} do
      conn = put conn, enr_admission_path(conn, :update, admission), admission: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Admission"
    end
  end
  ...
  describe "delete admission" do
    ...
    test "deletes chosen admission", %{conn: conn, admission: admission} do
      conn = delete conn, enr_admission_path(conn, :delete, admission)
      assert redirected_to(conn) == enr_admission_path(conn, :index)
      assert_error_sent 404, fn ->
        get conn, enr_admission_path(conn, :show, admission)
      end
    end
  end
